...
configure_request(XEvent *e)
{
  ...
  XConfigureWindow(d, ev->window, ev->value_mask, &(XWindowChanges) {
		   ...
		   ev->height,.sibling = ev->above,.stack_mode = ev->detail});
}
...
map_request(XEvent *e)
{
  ...
  XConfigureWindow(d, w, CWBorderWidth, &(XWindowChanges) {
...
create_bar(void)
{
  ...
  bar = XCreateWindow(d, root, TRAY_PADDING, 0, bar_width,
		      ...
		      DefaultVisual(d, s), CWOverrideRedirect | CWBackPixel,
		      &attr);
...
create_systray(void)
{
  ...
  systray = XCreateWindow(d, root, sw - TRAY_PADDING, 0,
			  TRAY_PADDING * 2, BAR_HEIGHT, 0, DefaultDepth(d,
									s),
...
update_systray(void)
{
  ...
  if (num_systray_icons > 0) {
    ...
    XMapWindow(d, systray);
    XMoveResizeWindow(d, systray, sw - tray_width, 0, tray_width, BAR_HEIGHT);

    for (i = 0; i < num_systray_icons; i++) {
      XMoveResizeWindow(d, systray_icons[i],
			TRAY_PADDING + i * (TRAY_ICON_SIZE +
					    TRAY_ICON_SPACING),
			(BAR_HEIGHT - TRAY_ICON_SIZE) / 2, TRAY_ICON_SIZE,
			TRAY_ICON_SIZE);
...
update_bar(void)
{
  ...
  bar_width = sw - (num_systray_icons > 0
		    ? (num_systray_icons *
		       (TRAY_ICON_SIZE + TRAY_ICON_SPACING)
